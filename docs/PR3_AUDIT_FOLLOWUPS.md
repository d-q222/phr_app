...
- L24 closed: both follow-ups landed — the root `conftest.py` guard anchored to
  `db.DATA_DIR` (`conftest.py:22-37`) and late-bound `db_path` defaults via
  `db._resolve_db_path` (`db.py:107-119`).
- L22 closed: the only remaining `delete_records_for_person` reference is the
  intentionally-historical changelog line (`docs/CODEBASE_SWEEP.md:434`); the
  `get_connection` walkthrough line now covers busy handling (`:61`).
...
| L10 | Insights retry-loop deadline/sleep-cap/3-attempt bound has no deterministic test | Only real-time tests exist (`test_insight_urlopen_timeout_stops_without_retry_or_sleep`, `test_insight_1113_makes_exactly_one_request_without_retry_or_sleep`); no fake-clock harness anywhere in `tests/` | Fake clock whose `sleep` advances `monotonic`; eligible-429 mock; assert request counts, non-increasing timeouts ≤ 30, sleeps `[3, 8]`, no request after deadline |

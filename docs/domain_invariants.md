...
|---|---|---|
| Reads | `WHERE person_id = ?` appended when `person_id` is not None | `db.list_records` |
| Creates | `person_id` force-set, overriding caller input | `services.create_item` |
| Updates | ownership folded into the write's own `WHERE id = ? AND person_id = ?`; `rowcount != 1` raises | `db._mutation_scope` + `db.update_record` |
| Deletes | same | `db._mutation_scope` + `db.delete_record` |
...
| Body map | reads exclusively through `services.list_items`, never `db` directly | `body_map_services.get_records_for_body_part` |
...
Ownership is a predicate on the mutation itself, not a prior probe, so there is no window between
checking and writing. `_mutation_scope` also refuses the ambiguous cases outright: a person-scoped
table with `person_id=None` raises, and so does a non-person-scoped table given one.
...
- `tests/test_basic.py::test_demo_database_loads_sample_data_without_touching_real_profiles`
- `tests/test_basic.py::test_profile_unlock_state_is_scoped_by_database`
- `tests/test_body_map_services.py::test_retrieval_returns_only_selected_person_records`
- `tests/test_body_map_services.py::test_every_record_adapter_applies_person_filter`
- `tests/test_body_map_ui.py::test_profile_change_clears_stale_body_state`
- `tests/test_body_map_ui.py::test_service_error_does_not_display_stale_profile_data`
...
| `db.update_record` / `delete_record` with `person_id=None` | `people` has no `person_id` column; needed by `services.update_person`/`delete_person` | scoping a non-person-scoped table raises `ValueError` (`tests/test_basic.py::test_create_record_rejects_person_id_for_unscoped_tables`) |
| `fhir.export_bundle(person_id=None)` | whole-record-set export for interoperability | UI offers "All profiles" only when **no** profile is locked — `app.page_import_export` |
...
- `tests/test_basic.py::test_json_restore_upserts_existing_records_without_deleting_children`
- `tests/test_basic.py::test_json_restore_rejects_malformed_backup_shapes`
- `tests/test_basic.py::test_json_restore_rejects_semantically_invalid_rows`
- `tests/test_basic.py::test_display_dataframe_keeps_unparseable_dates_unchanged`
- `tests/test_basic.py::test_fhir_import_skips_bad_patient_references_and_missing_required_fields`
- `tests/test_body_map_services.py::test_stored_mapping_takes_precedence_without_mutating_source`
- `tests/test_body_map_summary.py::test_summary_does_not_mutate_input`
...
- `tests/test_body_map_summary.py::test_raw_values_do_not_create_abnormal_flags`
- `tests/test_body_map_summary.py::test_newer_unflagged_record_preserves_old_flag_as_historical`
...
- `tests/test_basic.py::test_ai_insight_prompt_requires_safe_unobtrusive_suggestions`
- `tests/test_body_map_summary.py::test_summary_language_is_non_diagnostic`
...
- `tests/test_basic.py::test_ai_insight_retries_next_model_on_429`
- `tests/test_basic.py::test_ai_insight_transport_timeout_does_not_try_fallback_models`
- `tests/test_basic.py::test_ai_insight_account_error_does_not_retry_or_fallback`
- `tests/test_basic.py::test_ai_chat_timeout_is_model_independent_and_not_retried`
- `tests/test_basic.py::test_ai_chat_fallbacks_share_one_monotonic_deadline`
- `tests/test_basic.py::test_ai_chat_handles_rate_limit`
- `tests/test_basic.py::test_ai_chat_maps_auth_and_invalid_responses`
- `tests/test_basic.py::test_ai_chat_falls_back_when_primary_model_has_no_resource_package`
- `tests/test_body_map_services.py::test_database_error_is_not_reported_as_no_data` — a failure must not be rendered as "no data," which would read as clinically meaningful
...
Known code-level issue, tracked in `current_system.md`: `fhir.export_bundle` uses `datetime.utcnow()`, which is
